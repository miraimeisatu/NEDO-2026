# -*- coding: utf-8 -*-
"""[NEDO-4]IBM(量子自動分割) 確定版-100

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1CZu-lBecfNBJM8Csr9R65yuDCZgsRmm2
"""

# -*- coding: utf-8 -*-
"""[NEDO-4]IBM(量子自動分割版) 確定版
...
変更点 (v3.4-055 → v3.4-056):
  【高速化】QAOA 実行時間短縮（QAOA_P1: 770s / QAOA_P2: 1650s → 大幅削減目標）

  ■ 高速化ポイント① get_experiment_config: シミュレーター変更 + n_clusters自動増加
    - qaoa_method: "statevector" → "matrix_product_state"
      statevector は 2^N_sub の振幅を全保持（N_sub=19 → 524K状態）
      MPS はテンソルネットワーク近似で QAOA 浅い回路に最適
    - vrp_clusters を自動算出: N_sub ≤ 12 となる最小クラスタ数
      n_nodes=55: clusters=3 (N_sub=19) → clusters=5 (N_sub=11) に増加
      状態空間: 3×2^19=1.57M → 5×2^11=10,240 (約153倍削減)
    - qaoa_shots: 2048/1024 → 512 (サンプリング削減)
    - qaoa_maxiter: 150/100 → 80/60 (COBYLA反復削減)

  ■ 高速化ポイント② quantum_risk_predictor: バッチ実行
    - 旧: n_nodes 回の個別ジョブ実行 (トランスパイル×55回, job起動×55回)
    - 新: circuits_list を1回のバッチ実行 (job起動オーバーヘッド×1回)
    - shots: 1024 → 256 (H gate は p≈0.5 で安定、少数ショットで十分)

  ■ 高速化ポイント③ PHASE 2 QAOA パラメータ
    - reps: 2 → 1 (回路深さ半減、COBYLA評価1回あたりのコスト半減)
    - maxiter: 120 → 80 (反復削減)
    - shots: 1024 → 512
    - method: statevector → matrix_product_state

  ■ 高速化ポイント④ solve_qaoa_clustered: AerSampler 設定最適化
    - method="matrix_product_state" でテンソル近似演算
    - precision="single" (float32、デフォルト float64 の半分のメモリ)
    - max_parallel_threads=0 (全コア自動利用)

  期待効果 (n_nodes=55, n_clusters=5, N_sub=11):
    QAOA_P1: 770s → ~100s 以下
    QAOA_P2: 1650s → ~150s 以下
"""
...
# ============================================================
# ----------------------------------------------------------------
# 【高速化ポイント①】 get_experiment_config
#
#  変更内容:
#    a) qaoa_method: "statevector" → "matrix_product_state"
#       - statevector は 2^N_sub 個の振幅を全メモリに保持するため
#         N_sub=19 では 2^19=524,288 振幅 → 大きなメモリ消費と演算コスト
#       - matrix_product_state (MPS) はテンソルネットワーク近似で
#         エンタングルメントが弱い回路（QAOA浅い層）で特に高速
#
#    b) vrp_clusters: 増加させて N_sub を削減
#       - n_nodes=55 の場合: 旧 clusters=3 → N_sub=19(2^19=524K状態)
#                           新 clusters=6 → N_sub=10(2^10=1K状態)
#       - N_sub が 2 減るだけで状態空間が 1/4 に削減
#       - 目安: N_sub ≤ 12 を維持する clusters 数を自動算出
#
#    c) qaoa_shots: 2048→512 / 1024→512
#       - 量子サンプリング数を削減（精度は若干低下するが実用レベル維持）
#       - ショット数はCOBYLAのnoiseに対する感度に影響するが
#         N_sub が小さければ landscape が単純で少ないサンプルで収束
#
#    d) qaoa_maxiter: 150→80 / 100→60
#       - COBYLA の最大反復数を削減
#       - 経験則: N_sub ≤ 12 では 60〜80 回で十分収束する
# ----------------------------------------------------------------
N_SUB_TARGET = 12   # ★ N_sub をこの値以下に抑えるための目標上限
MAX_QUBITS = 20     # ※BIt上限数、ここを調整

def get_experiment_config(n_nodes):
    if n_nodes < MAX_QUBITS:
        # n_nodes<20 は元から小さいのでクラスタ分割不要
        return {
            "use_cluster":   False,
            "qaoa_method":   "matrix_product_state",   # ← statevector より高速
            "qaoa_shots":    512,                       # ← 2048→512
            "qaoa_maxiter":  80,                        # ← 150→80
            "qaoa_reps":     1,
            "vrp_time_limit": 120,
            "vrp_clusters":  2,
        }
    else:
        # ★ N_sub ≤ N_SUB_TARGET となる最小クラスタ数を自動決定
        auto_clusters = math.ceil(n_nodes / N_SUB_TARGET)
        # KMeans の安定性確保のため最低クラスタ数を保証
        min_clusters  = 2 if n_nodes <= 35 else 3
        opt_clusters  = max(min_clusters, auto_clusters)
        # 算出結果をログ出力（後続の print で確認可能）
        computed_N_sub = math.ceil(n_nodes / opt_clusters)
        print(f"  [高速化] N_sub目標≤{N_SUB_TARGET}: "
              f"n_clusters={opt_clusters} → N_sub={computed_N_sub} "
              f"(状態空間: 2^{computed_N_sub}={2**computed_N_sub:,})")

        return {
            "use_cluster":   True,
            "qaoa_method":   "matrix_product_state",   # ← statevector より高速
            "qaoa_shots":    512,                       # ← 1024→512
            "qaoa_maxiter":  60,                        # ← 100→60
            "qaoa_reps":     1,
            "vrp_clusters":  opt_clusters,              # ← 自動増加で N_sub 削減
...
def solve_qaoa_clustered(scores_list, label_prefix, qaoa_config, n_cl):
    ...
    【高速化ポイント④】 AerSampler の設定最適化
      - max_parallel_threads=0 → Aer が自動でコア数を検出して並列実行
      - method="matrix_product_state": テンソルネットワーク近似
        N_sub ≤ 12 程度ではエンタングルメントが小さく MPS が statevector より
        圧倒的に速い（状態空間を全保持せず必要部分のみ計算）
      - precision="single": float32 使用（デフォルト float64 の半分のメモリ）
    """
    N        = len(scores_list)
    ...
    # ★ AerSampler: matrix_product_state + 並列スレッド自動設定
    sampler = AerSampler(
        options={
            "backend_options": {
                "method":               qaoa_config["qaoa_method"],  # MPS
                "precision":            "single",      # float32で省メモリ
                "max_parallel_threads": 0,             # 全コアを自動利用
            },
...
# PHASE 2 ③  量子リスク予測関数
# ----------------------------------------------------------
def quantum_risk_predictor(df_in, shots=256):
    # ----------------------------------------------------------------
    # 【高速化ポイント②】 quantum_risk_predictor のバッチ実行
    #
    #  旧実装: n_nodes 回の個別ジョブ実行
    #    for _ in range(n_nodes):  # ← 55回ループ
    #        job = backend.run(transpile(qc, backend), shots=shots)
    #  → トランスパイル×55回 + ジョブ起動オーバーヘッド×55回
    #
    #  新実装: n_nodes 個の回路をリスト渡しで1回のバッチ実行
    #    job = backend.run(circuits_list, shots=shots)
    #  → トランスパイル×1回（まとめて） + ジョブ起動×1回
    #
    #  また shots=1024→256 に削減
    #  (H gate の確率はほぼ 0.5 で安定しており少数ショットで十分)
    # ----------------------------------------------------------------
    backend = AerSimulator()
    n       = len(df_in)

    # n_nodes 個の同一回路をまとめてトランスパイル＆バッチ実行
    qc = QuantumCircuit(1)
    qc.h(0)
    qc.measure_all()
    transpiled_single = transpile(qc, backend)
    circuits_batch    = [transpiled_single] * n       # 同一回路を n 個

    job    = backend.run(circuits_batch, shots=shots)  # ★ バッチ一括実行
    result = job.result()
...
# ★ QAOA クラスタ分割実行
# ----------------------------------------------------------------
# 【高速化ポイント③】 PHASE 2 QAOA パラメータ最適化
#
#  旧設定: reps=2, maxiter=120, shots=1024
#  新設定: reps=1, maxiter=80,  shots=512
#
#  reps=2 → reps=1:
#    - QAOA の回路深さは reps に比例して増加
#    - reps=2 の回路はパラメータ数 2×n_qubits, reps=1 の2倍深い
#    - statevector では深い回路ほど演算コスト増大
#    - MPS では特に顕著（テンソル縮約コストが回路深さに敏感）
#    - N_sub ≤ 12 のスケールでは reps=1 でも解の品質は十分
#
#  maxiter 120→80:
#    - COBYLA 評価回数削減 → QAOA ショット数×maxiter 分の量子回路実行削減
#    - P2 の landscape は P1 よりスムーズ（重みスケールが大きいため）
#      → 少ない反復で収束しやすい
# ----------------------------------------------------------------
config_p2 = dict(config)
config_p2["qaoa_reps"]    = 1    # ← 2→1: 回路深さ半減
config_p2["qaoa_maxiter"] = 80   # ← 120→80: COBYLA反復削減
config_p2["qaoa_shots"]   = 512  # ← 1024→512: サンプリング削減
config_p2["qaoa_method"]  = "matrix_product_state"  # ← statevector→MPS
